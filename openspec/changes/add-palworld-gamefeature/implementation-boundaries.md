...
| **LyraGame** | `Source/LyraGame/` | 四项缺陷修复，以及一个可跨 Experience 复用的通用 Inventory 主动丢弃原语：InventoryManager owning-client RPC、Authority/归属校验、事务性生成、跨 Actor Outer 实例复制与通用世界拾取 Actor。 | 不接受 ShooterGame/Palworld 玩法规则或视觉资产；除该通用 Inventory 原语外不新增功能扩展。 |
| **Primary Data Asset** | ShooterCore 插件 `Content/` | `GameFeatureData`、`ExperienceDefinition`、`UserFacingExperience`、`PawnData` 等作为配置实例与软引用存在，可被 Asset Manager 扫描。 | 不承载蓝图事件图逻辑；不作为持久软引用目标以外的用途。 |
| **视觉资产** | ShooterCore 插件 `Content/` | UMG 布局、模型、材质、贴图、动画蓝图、地图等由用户创建/提供，实现本变更定义的视图契约。 | 替换视觉资产不得要求改动会话、阶段、队伍、背包或装备服务；界面不得直接持有这些服务的权威写入路径。 |
...
3. 改 `LyraGame` 前先对照「四项缺陷修复 + 通用 Inventory 主动丢弃原语」清单，不在清单内 → 驳回。
